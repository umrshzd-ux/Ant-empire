...
// Three.js setup (renderer, lights, raycaster)
    ...
    try {
      ...
      if (!state.weatherActive) {
        ...
        if (state.weatherTimer <= 0) {
          ...
          state.weatherTimeLeft = BAL.weatherDuration;
      ...
      for (var i = eggMs.length - 1; i >= 0; i--) {
        ...
        egg.mat.emissive.setHex(0xffcc66);
        egg.mat.emissiveIntensity = u * 0.6;
      ...
      for (var i = hatchFx.length - 1; i >= 0; i--) {
        ...
        for (var ci = 0; ci < fx.group.children.length; ci++) {
          ...
          s.position.x += s.userData.dir.x * dt * 1.2;
          s.position.y += s.userData.dir.y * dt * 1.2;
          s.position.z += s.userData.dir.z * dt * 1.2;
      ...
      if (researchChamberGroup && researchChamberGroup.children) {
        ...
        for (var oi = 0; oi < researchChamberGroup.children.length; oi++) {
          ...
          orb.position.x = Math.cos(a) * 0.6;
          orb.position.z = Math.sin(a) * 0.6;
          orb.position.y = Math.sin(time * 2 + oi) * 0.15;
        }
      }
...
function initGameSystems() {
  ...
  for (var wi = 0; wi < Math.min(state.workerCount, BAL.maxRenderedAnts); wi++) {
    var w = createWorker(false);
    if (w) workers.push(w);
  }
  state.virtualWorkers = Math.max(0, state.workerCount - workers.length);
  // Create initial soldiers
  for (var si = 0; si < state.soldierCount; si++) {
    var chX = BAL.soldierRowStart + TX + 5 + si * 3.5;
    spawnSoldier(chX);
  }
  ...
  if (state.nestEvolutionLevel > 0) {
    for (var tier = 1; tier <= state.nestEvolutionLevel; tier++) evolveNestVisuals(tier, true);
  }
  // Zone visuals
  var savedZone = ZONE_CONFIG[state.currentZone];
  if (savedZone) {
    scene.background = new THREE.Color(savedZone.bg);
    scene.fog = new THREE.Fog(savedZone.fog, 20, 80);
  }
  ...
  // Offline progress or daily login
...
function renderSlots() {
  ...
  for (var i = 0; i < slots.length; i++) {
    ...
    if (sl.hasData) {
      ...
      html += '<div class="slot-name">🐜 ' + sl.name + '</div><div class="slot-info">Lv ' + sl.level + ' | Prestige ' + sl.prestige + ' | Ascension ' + sl.ascension + ' | ' + timeAgo + '</div><div class="slot-info" style="margin-top:4px;"><span onclick="event.stopPropagation();deleteSlot(' + i + ')" style="color:#ff6666;cursor:pointer;">🗑️ Delete</span></div>';
    } else {
...
try {
  GameSettings.init();
  AudioManager.init();
  var lastSlot = SaveManager.getLastSlot();
  if (lastSlot >= 0) {
    var data = SaveManager.loadGame(lastSlot);
    if (data) {
  ...
  document.getElementById('loading').innerHTML = '<div class="title">⚠️ Error Loading</div><div>Your save data may be corrupted.</div><button onclick="localStorage.clear();location.reload();" style="margin-top:15px;padding:10px 20px;background:#d6a85a;border:none;border-radius:8px;color:#1a1208;font-weight:700;cursor:pointer;">Reset & Retry</button>';
  return;
}
...
console.log("🐜 Ant Empire v19 — Modular polished edition");
